...
import dev.gtnh.intellij.generation.config.GtnhConfigEntryGenerator
import dev.gtnh.intellij.project.GtnhProjectDetector
import dev.gtnh.intellij.ui.GtnhGenerationPreview

class NewGtnhConfigEntryAction : AnAction() {
    ...
    override fun actionPerformed(event: AnActionEvent) {
        ...
        val field = Messages.showInputDialog(project, "Config field name", "New GTNH Config Entry", null) ?: return
        val plan = runCatching { GtnhConfigEntryGenerator.plan(file.packageName, field) }.getOrElse {
        ...
        else if (GtnhGenerationPreview(project, plan).showAndGet()) service.apply(plan)
    }
}
